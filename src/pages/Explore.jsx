...
const Explore = () => {
  ...
  const results = destinations.filter((destination) => {
    ...
    const matchesPlace = !place || destination.placename === place;
    const searchable = `${destination.placename} ${destination.location.state} ${destination.location.city} ${destination.category}`.toLowerCase();
  ...
  return (
    <main className="relative min-h-screen bg-green px-5 py-16 md:px-16 z-auto ">
      <div className="absolute mx-auto max-w-6xl">
        ...
        {results.length > 0 ? (
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {results.map((destination) => (
              <Link key={destination.placename} to={`/place/${encodeURIComponent(destination.placename)}`} className="group overflow-hidden rounded-2xl bg-white shadow-md transition hover:-translate-y-1 hover:shadow-xl">
                <div className="h-52 overflow-hidden bg-zinc-200">
                  <img src={destination.images?.mainBanner} alt={destination.placename} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                </div>
                <div className="p-5">
                  <p className="text-xs font-bold uppercase tracking-wider text-orange-600">{destination.category}</p>
                  <h2 className="mt-2 font-serif text-2xl font-bold text-zinc-900">{destination.placename}</h2>
